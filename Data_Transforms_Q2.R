...
maxflMultiplier <- as.numeric(mean(head(maxfl.Mean$percentofFloor,7000)))
naMAXfl <- as.numeric(df[which(is.na(df$max_floor)),3])
df[which(is.na(df$max_floor)),6] <- naMAXfl*maxflMultiplier
##########
df[which(df$year == 2011),]
########## life sq
...
naLIFEsq <- as.numeric(df[which(is.na(df$life_sq)),3])
df[which(is.na(df$life_sq)),4] <- naLIFEsq*lifesqMultiplier
...
naKITCHsq <- as.numeric(df[which(is.na(df$kitch_sq)),3])
df[which(is.na(df$kitch_sq)),10] <- naKITCHsq*kitchsqMultiplier
...
naNUMrm <- as.numeric(df[which(is.na(df$num_room)),3])
df[which(is.na(df$num_room)),9] <- naNUMrm*numRmMultiplier
#df <- df[which(df$num_room < 3),]
##########
df[which(df$year == 2011),]
########## preschool quota
...
naPREk <- as.numeric(df[which(is.na(df$preschool_quota)),26])
df[which(is.na(df$preschool_quota)),16] <- naPREk*preKquotaMultiplier
