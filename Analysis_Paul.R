...
model.Allvar <- lm(log(price_doc) ~ id + timestamp + full_sq +	life_sq + floor + max_floor + material + build_year + num_room + kitch_sq + product_type
                    ...
                    + big_market_raion + full_all + `0_6_all` + `7_14_all` + `0_17_all` + `16_29_all` + `0_13_all` + build_count_block + build_count_wood 
...
model.Forward <- stepAIC(model2.forward.Start, direction = "forward", trace = F, scope = formula(model.Allvar))
